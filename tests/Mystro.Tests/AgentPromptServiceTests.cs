...
public class AgentPromptServiceTests : IDisposable
{
    ...
    {
        var agentsRoot = Path.Combine(_tempDirectory.FullName, ".mystro", "agents");
    ...
    {
        Directory.CreateDirectory(Path.Combine(_tempDirectory.FullName, ".mystro"));
        Directory.SetCurrentDirectory(_tempDirectory.FullName);
        var sut = new AgentPromptService(new FileService(new System.IO.Abstractions.FileSystem()));
        ...
        Assert.Contains(".mystro/agents/lead.md", ex.Message.Replace('\\', '/'), StringComparison.Ordinal);
    }

    [Fact]
    public void LoadEngineerPromptTemplate_Throws_WhenEmpty()
    {
        var agentsDirectory = Path.Combine(_tempDirectory.FullName, ".mystro", "agents");
    ...
    {
        var agentsDirectory = Path.Combine(_tempDirectory.FullName, ".mystro", "promptopt", "bundles", "bundle-1", "agents");
        Directory.CreateDirectory(agentsDirectory);
        ...
        var prompt = sut.LoadEngineerPromptTemplate(".mystro/promptopt/bundles/bundle-1/agents");
    ...
    public void LoadEngineerPromptTemplate_Throws_WhenToolsMissing()
    {
        ...
        var ex = Assert.Throws<InvalidOperationException>(() => sut.LoadEngineerPromptTemplate());
        Assert.Contains("missing", ex.Message, StringComparison.OrdinalIgnoreCase);
        Assert.Contains(".mystro/agents/tools.md", ex.Message.Replace('\\', '/'), StringComparison.Ordinal);
    }
