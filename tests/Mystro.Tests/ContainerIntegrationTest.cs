...
public class ContainerIntegrationTest : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var validator = new Mystro.Infrastructure.DockerEnvironmentValidator();
        try
        {
        ...
        var localSdkPath = EnsureProjectOutput(repoRoot, "Mystro.SDK", configuration);
        var localDomainPath = EnsureProjectOutput(repoRoot, "Mystro.Domain", configuration);
    ...
    public async Task Bootstrap_ShouldInjectSDK_And_ExecuteScript()
    {
        ...
        var scriptContent = $@"
#r ""{SdkPath}/Mystro.SDK.dll""
#r ""{DomainPath}/Mystro.Domain.dll"" 
using Mystro.SDK;

var sdk = new MystroSDK(new ConsoleJsonSdkEventSink());
sdk.WriteFile(""tmp/bootstrapped.txt"", ""Hello from Mystro!"");
";
        ...
        Assert.Contains("Hello from Mystro!", fileCheck.Stdout);
    }
