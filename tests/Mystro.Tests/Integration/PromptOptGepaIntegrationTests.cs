...
public class PromptOptGepaIntegrationTests
{
    ...
    public void RunGepa_MutatesPracticeFromStubbedFeedback()
    {
        ...
        var tempRoot = Path.Combine(Path.GetTempPath(), "mystro_gepa_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempRoot);

        try
        {
            var promptOptRoot = Path.Combine(tempRoot, ".mystro", "promptopt");
            ...
            var cliDll = EnsureProjectOutput(repoRoot, "Mystro.PromptOpt.CLI", ResolveConfiguration());
