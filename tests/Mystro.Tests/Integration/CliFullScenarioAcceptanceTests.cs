...
namespace Mystro.Tests.Integration;

public class CliFullScenarioAcceptanceTests
{
    private const string RealLlmToggleEnvVar = "MYSTRO_ACCEPTANCE_USE_REAL_LLM";
    ...
    private static async Task<AcceptanceRunResult> RunScenarioAsync(AcceptanceScenario scenario, AgentConfiguration agentConfiguration)
    {
        ...
        var cliDllPath = EnsureProjectOutput(repositoryRoot, "Mystro.Presentation.CLI", configuration);

        var tempRoot = Path.Combine(Path.GetTempPath(), "mystro_cli_acceptance_" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            var sourcePracticesPath = Path.Combine(repositoryRoot, ".mystro", "practices");
            var targetPracticesPath = Path.Combine(repoClonePath, ".mystro", "practices");
            if (Directory.Exists(sourcePracticesPath))
            {
                CopyDirectory(sourcePracticesPath, targetPracticesPath);
            }

            var sourceAgentsPath = Path.Combine(repositoryRoot, ".mystro", "agents");
            var targetAgentsPath = Path.Combine(repoClonePath, ".mystro", "agents");
    ...
    {
        var runRootPath = Path.Combine(repoClonePath, ".mystro", "promptopt", "runs");
    ...
    private static string FindRepoRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(directory.FullName, "Mystro.sln")))
            {
        ...
        throw new InvalidOperationException("Could not locate Mystro.sln from test base directory.");
    }
