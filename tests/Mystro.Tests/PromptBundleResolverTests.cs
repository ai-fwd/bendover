...
public class PromptBundleResolverTests : IDisposable
{
    ...
    {
        _tempDir = Directory.CreateTempSubdirectory("MystroTests_");
    ...
    public void Resolve_ShouldReturnDefaultPath_WhenActiveJsonMissing()
    {
        ...
        // Assert
        var expected = Path.Combine(_repoRoot, ".mystro", "promptopt", "practices");
    ...
    {
        // Arrange
        var promptOptDir = Directory.CreateDirectory(Path.Combine(_repoRoot, ".mystro", "promptopt"));
        ...
        // Assert
        var expected = Path.Combine(_repoRoot, ".mystro", "promptopt", "bundles", "test_bundle", "practices");
        Assert.Equal(expected, result);
    }
