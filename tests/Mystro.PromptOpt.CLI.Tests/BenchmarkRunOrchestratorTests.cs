    ...
    public async Task RunAsync_ShouldUseBundlePathAndTaskPathAndEmitArtifacts()
    {
        ...
        _agentOrchestratorMock.Verify(
            x => x.RunAsync(
                ...
                It.Is<string?>(path => path!.Replace('\\', '/') == ".mystro/promptopt/bundles/bundle-456/agents")),
            Times.Once);
        ...
        _agentOrchestratorMock.Setup(x => x.RunAsync(taskText, It.IsAny<IReadOnlyCollection<Practice>>(), It.IsAny<string?>()))
            .Callback(() =>
            {
                ...
                sawCopiedLeadTemplate = paths.Any(path =>
                    path.EndsWith(".mystro/promptopt/bundles/bundle-456/agents/lead.md", StringComparison.Ordinal));
                sawCopiedEngineerTemplate = paths.Any(path =>
                    path.EndsWith(".mystro/promptopt/bundles/bundle-456/agents/engineer.md", StringComparison.Ordinal));
                sawCopiedToolsTemplate = paths.Any(path =>
                    path.EndsWith(".mystro/promptopt/bundles/bundle-456/agents/tools.md", StringComparison.Ordinal));
            })
            .Returns(Task.CompletedTask);
