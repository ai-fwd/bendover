...
public class PromptOptRunRecorder : IPromptOptRunRecorder
{
    ...
    public async Task<string> StartRunAsync(string goal, string baseCommit, string bundleId)
    {
        ...
        _runId = $"{DateTime.UtcNow:yyyyMMdd_HHmmss}_{shortGuid}";

        // Create run directory relative to repo root
        // Assuming current working directory is repo root or we can find it.
        // For now, using .bendover/promptopt/runs/ relative to CWD as per typical CLI usage.
        _runDir = Path.Combine(".bendover", "promptopt", "runs", _runId);
    ...
    public async Task FinalizeRunAsync()
    {
        ...
        try
        {
            ...
            if (File.Exists(Path.Combine(_runDir, "git_diff.patch")))
            {
