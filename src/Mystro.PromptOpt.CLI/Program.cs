...
public static class Program
{
    public static Task<int> Main(string[] args)
    {
        if (args.Length > 0 && string.Equals(args[0], "serve-chatgpt-proxy", StringComparison.OrdinalIgnoreCase))
        {
            ...
            proxyApp.Configure(config => config.SetApplicationName("mystro-promptopt"));
        ...
        app.Configure(config => config.SetApplicationName("mystro-promptopt"));
...
public class RunCommandSettings : CommandSettings
{
    ...
    [Description("Score an existing run under .mystro/promptopt/runs/<ID>.")]
