...
public class RunScoringOrchestrator
{
        ...
        IPromptOptCliStatusSink? statusSink)
    {
        ...
        var runsRoot = _fileSystem.Path.Combine(repoRoot, ".mystro", "promptopt", "runs");
    ...
    private string ResolveBundlePath(string repoRoot, string runDirectory, string? bundleOverridePath)
    {
        ...
        {
            return _fileSystem.Path.Combine(repoRoot, ".mystro");
        }

        return _fileSystem.Path.Combine(repoRoot, ".mystro", "promptopt", "bundles", bundleId);
    }
